...
import { Building2, Plus, MapPin, X, Home, Pencil, Trash2 } from 'lucide-react'
...
const EMPTY_FORM = { name: '', address: '', city: 'Montevideo', notes: '' }
...
export default function PropertiesPage() {
    ...
    const openEdit = (prop: Property) => {
        ...
        setEditForm({ name: prop.name, address: prop.address, city: prop.city, notes: prop.notes || '' })
        setEditError(null)
    }
        ...
        if (res.ok) {
            setProperties(prev =>
                prev.map(p => p.id === selectedProperty.id
                    ? { ...p, _count: { units: p._count.units + 1 } } : p)
            )
            setShowUnitModal(false); setUnitLabel(''); setSelectedProperty(null)
        }
        setSavingUnit(false)
    }
        ...
        <div className="p-8 space-y-8 animate-in fade-in duration-500">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-3">
                        <Building2 className="w-8 h-8 text-blue-600" />Propiedades
                    </h1>
                        ...
                        <div key={prop.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover-lift relative overflow-hidden group">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <button onClick={() => setDeletingProperty(prop)}
                            ...
                            <div className="space-y-3 mb-6">
                                ...
                                {prop.notes && <p className="text-xs text-slate-400 italic">{prop.notes}</p>}
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            {[
                                { label: 'Nombre *', key: 'name', placeholder: 'Ej: Edificio Bulevar', required: true },
                                { label: 'Dirección *', key: 'address', placeholder: 'Ej: Bulevar Artigas 1234', required: true },
                                { label: 'Ciudad *', key: 'city', placeholder: 'Ej: Montevideo', required: true },
            ...
            {editingProperty && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-900">Editar Propiedad</h2>
                            <button onClick={() => setEditingProperty(null)} className="text-slate-400 hover:text-slate-700"><X className="w-5 h-5" /></button>
                        </div>
                        <form onSubmit={handleEdit} className="space-y-4">
                            {[
                                { label: 'Nombre *', key: 'name', required: true },
                                { label: 'Dirección *', key: 'address', required: true },
                                { label: 'Ciudad *', key: 'city', required: true },
                            ].map(({ label, key, required }) => (
                                <div key={key}>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">{label}</label>
                                    <input required={required} value={editForm[key as keyof typeof editForm]}
            ...
            {deletingProperty && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
                        <h2 className="text-lg font-bold text-slate-900 mb-2">Eliminar propiedad</h2>
                        <p className="text-slate-600 text-sm mb-6">
                            ¿Eliminar <span className="font-semibold">{deletingProperty.name}</span>? Esta acción no se puede deshacer.
                        </p>
                        <div className="flex gap-3">
                            <button onClick={() => setDeletingProperty(null)}
                                className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-700 font-medium hover:bg-slate-50">Cancelar</button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setShowUnitModal(false)} className="text-slate-400 hover:text-slate-700"><X className="w-5 h-5" /></button>
                        </div>
                        ...
                        <form onSubmit={handleAddUnit} className="space-y-4">
                            <div>
                                ...
                                <input required value={unitLabel} onChange={e => setUnitLabel(e.target.value)}
                                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="Ej: Apto 101, Local A..." />
                            </div>
                            <div className="flex gap-3">
